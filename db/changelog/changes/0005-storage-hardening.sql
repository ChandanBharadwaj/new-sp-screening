...
-- constraints, the `ADD CONSTRAINT` calls will fail. Operator dedupe SQL lives at
-- the bottom of this file for copy/paste.

--changeset screening:0005-uq-sanctioned-commodity-source-recid
-- Idempotency for sanctions ingesters. source_record_id is nullable; rows without
-- one continue to insert each run (no stable identity).
ALTER TABLE sanctioned_commodity
    ADD CONSTRAINT uq_sanctioned_commodity_source_recid
    UNIQUE (source, source_record_id);
--rollback ALTER TABLE sanctioned_commodity DROP CONSTRAINT IF EXISTS uq_sanctioned_commodity_source_recid;

--changeset screening:0005-uq-country-rule
-- NULLS NOT DISTINCT so (NULL, 'RU', 42, 'blocked') collides with itself on the second ingest.
ALTER TABLE country_rule
    ADD CONSTRAINT uq_country_rule
    UNIQUE NULLS NOT DISTINCT (origin_iso, destination_iso, sanctioned_commodity_id, restriction_type);
--rollback ALTER TABLE country_rule DROP CONSTRAINT IF EXISTS uq_country_rule;

--changeset screening:0005-uq-hs-training-example
-- A (source, source_id, hs_code) triplet identifies a ruling/example uniquely;
-- without this, CROSS re-scrapes accumulate duplicates.
ALTER TABLE hs_training_example
    ADD CONSTRAINT uq_hs_training_example
    UNIQUE NULLS NOT DISTINCT (source, source_id, hs_code);
--rollback ALTER TABLE hs_training_example DROP CONSTRAINT IF EXISTS uq_hs_training_example;

--changeset screening:0005-idx-sanctioned-commodity-hs-codes-gin
...
--rollback DROP INDEX IF EXISTS ix_sanctioned_commodity_hs_codes_gin;

--changeset screening:0005-idx-sanctioned-commodity-effective
...
--rollback DROP INDEX IF EXISTS ix_sanctioned_commodity_effective;
