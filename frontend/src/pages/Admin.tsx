...
function SourceCard({ source }: { source: Source }) {
  ...
  const runLabel = source.auto_download ? "Reload from internet" : "Run";
  ...
  return (
    <div className="border rounded-lg p-4 bg-white shadow-sm">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {source.auto_download ? (
              <span className="px-1.5 py-0.5 rounded text-xs bg-indigo-100 text-indigo-800">
                Internet source
              </span>
            ) : (
      ...
      {lr?.status === "running" && lr?.id && (
        <div className="mt-3">
          <LogStream runTable="refdata_run" runId={lr.id} height="10rem" />
        </div>
      )}
    </div>
  );
